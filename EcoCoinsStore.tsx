...
import { Coins, ShoppingBag, Star, Gift, Trophy, TreePine, Sparkles, Crown, Award, Heart } from 'lucide-react';
...
const EcoCoinsStore = () => {
  const [activeCategory, setActiveCategory] = useState<'badges' | 'city' | 'rewards' | 'premium'>('badges');
  ...
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      case 'legendary': return 'from-yellow-400 to-orange-500 text-yellow-900';
      case 'rare': return 'from-purple-400 to-blue-500 text-purple-900';
      case 'common': return 'from-green-400 to-blue-400 text-green-900';
  ...
  const handlePurchase = (item: StoreItem) => {
    if (ecoCoins >= item.cost) {
      ...
      toast({
        ...
        description: `You need ${item.cost - ecoCoins} more eco-coins for this item.`,
        variant: "destructive"
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold text-gradient-eco mb-2">Eco-Coins Store</h2>
        <p className="text-muted-foreground">
          Spend your hard-earned eco-coins on badges, city upgrades, and real rewards!
        </p>
      </div>
      ...
      <Card className="card-eco">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-bold text-gradient-eco">{ecoCoins.toLocaleString()}</h3>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredItems.map((item) => (
          <Card
            key={item.id}
            className={`card-eco border-2 ${getRarityBorder(item.rarity)} relative overflow-hidden`}
          >
            {/* Rarity Badge */}
            <div className={`absolute top-3 right-3 px-2 py-1 rounded-full text-xs font-semibold bg-gradient-to-r ${getRarityColor(item.rarity)}`}>
              {item.rarity}
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="text-center">
                <div className={`h-16 w-16 mx-auto rounded-2xl bg-gradient-to-r ${getRarityColor(item.rarity)} p-0.5`}>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">eco-coins</span>
                </div>

                <Button
                  className="w-full btn-eco"
                  onClick={() => handlePurchase(item)}
                  disabled={ecoCoins < item.cost || (item.stock !== undefined && item.stock === 0)}
                >
                  {ecoCoins < item.cost ? 'Not Enough Coins' : 
      ...
      <Card className="card-eco bg-gradient-to-r from-primary/5 to-primary-glow/5">
        <div className="text-center py-6">
          ...
          <h3 className="text-xl font-bold text-gradient-eco mb-2">Weekly Special!</h3>
        ...
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span>Complete daily eco-actions: <strong>10-50 coins</strong></span>
            </div>
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 bg-primary rounded-full" />
              <span>Finish learning modules: <strong>25 coins</strong></span>
            </div>
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 bg-primary rounded-full" />
              <span>Get verified peer actions: <strong>30 coins</strong></span>
            </div>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 bg-primary rounded-full" />
              <span>Weekly streak bonus: <strong>100 coins</strong></span>
            </div>
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 bg-primary rounded-full" />
              <span>Top 3 in leaderboard: <strong>200 coins</strong></span>
            </div>
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 bg-primary rounded-full" />
              <span>Share eco-stories: <strong>15 coins</strong></span>
            </div>
          </div>
        </div>
      </Card>
